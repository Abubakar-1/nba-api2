...
const OldBpfReceipt: FunctionalComponent = () => {
  const navigate = useNavigate();
  const { oldReceiptInfo } = OldTransactionContext.useContainer();
  const { user } = AuthContext.useContainer();
  const componentRef = useRef(null);

  const enrichedData = oldReceiptInfo;
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `${oldReceiptInfo?.name}-receipt`,
    pageStyle: pageStyle,
  });

  useEffect(() => {
    !oldReceiptInfo && navigate(-1);
  }, []);

  return (
    <div className="px-4">
      <PageTitle title="BPF Receipt" />
